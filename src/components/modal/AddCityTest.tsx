...
import { FlaskConical, X, Plus, Trash2, Loader2 } from 'lucide-react';
import { cityTestSchema, CityTestFormValues } from '@/validation/validation';
import { useCreateCityTestMutation } from '@/redux/features/app/app.api';
import { toast } from 'sonner';

interface AddCityTestProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function AddCityTest({ isOpen, onClose }: AddCityTestProps) {
  const [createCityTest, { isLoading }] = useCreateCityTestMutation();
  ...
  const handleFormSubmit = async (values: CityTestFormValues) => {
    try {
      ...
      toast.success('City test created successfully!');
      reset();
      setNotePromptsText('');
      setQuestionPromptsText('');
      onClose();
    } catch (err: any) {
      console.error('Failed to create city test:', err);
      const backendMsg =
        err?.data?.id?.[0] ||
        err?.data?.detail ||
        err?.data?.message ||
        'Failed to create city test';
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              />
              {errors.id && (
                <p className="text-[11px] text-red-500 mt-0.5 font-medium">{errors.id.message}</p>
              )}
            </div>
